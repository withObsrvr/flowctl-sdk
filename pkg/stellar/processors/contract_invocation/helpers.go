...
func extractArguments(args []xdr.ScVal) ([]xdr.ScVal, []json.RawMessage, map[string]interface{}, error) {
	...
	for i, arg := range args {
		...
		decoded, err := ConvertScValToJSON(arg)
		if err != nil {
			return argsCopy, jsonRawArgs, decodedArgs, fmt.Errorf("error decoding argument %d: %w", i, err)
		}

		// Serialize to JSON for raw representation
		jsonBytes, err := json.Marshal(decoded)
		if err != nil {
			return argsCopy, jsonRawArgs, decodedArgs, fmt.Errorf("error marshaling argument %d: %w", i, err)
		}

		jsonRawArgs = append(jsonRawArgs, json.RawMessage(jsonBytes))
		decodedArgs[fmt.Sprintf("arg_%d", i)] = decoded
	}
...
func convertScAddress(addr xdr.ScAddress) (interface{}, error) {
	...
	default:
		return nil, fmt.Errorf("unsupported ScAddress type: %v", addr.Type)
	}
}
